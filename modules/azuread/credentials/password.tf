...
resource "azuread_application_password" "key" {
  ...
  application_id = try(var.resources.application.id, null)
...
resource "azurerm_key_vault_secret" "client_secret" {
  for_each = {
    ...
    if key != "lz_key" && can(value.secret_prefix) && try(var.settings.azuread_application, null) != null && lower(local.password_type) == "password"
  }
